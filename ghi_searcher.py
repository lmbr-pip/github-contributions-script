...
class GitHubIssuesSearcher(object):
    ...
    def pr_ranged_reviewed_query(self, user: str, organization: str, range_start: str, range_end: str) -> str:
        """
        Builds a query to find all the reviewed PRs by the user in the provided date range. Excludes PRs opened
        by the user. To find those use merges option which uses pr_ranged_merged_query

        Search uses not to exclude PRs generated by author
        ...
        :return: query to use with search api
        """
        return f'is:pr+reviewed-by:{user}+-author:{user}+user:{organization}+created:{range_start}..{range_end}'
